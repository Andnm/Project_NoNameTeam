...
Future<Uint8List?> getUint8ListCompressImage(Uint8List u8List, int width, int height, int quality) async {
  final result = await FlutterImageCompress.compressWithList(u8List, quality: quality, minWidth: width, minHeight: height);
  return result;
}

Future<PhotoInfo> compressImageWithUint8List(Uint8List mU8List, {double maxWidth = 720, double maxHeight = 1080}) async {
  ...
  imageFile.resolve(new ImageConfiguration()).addListener(ImageStreamListener((info, _) async {
        ...
        Uint8List? u8List = await getUint8ListCompressImage(mU8List, maxWidth.toInt(), maxHeight.toInt(), getQuality(length));
        var photoInfo = PhotoInfo(u8List: u8List, isVertical: isVertical, ratio: ratio);
